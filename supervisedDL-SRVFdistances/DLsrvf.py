    ...
    for j in range(0, e):
        ...
        for i in range(0, trainsize):
            ...
            for k in range(0, reparamn):
                trData[i * reparamn + k, 0:length, 0:dim] = randomCurveFromShapeClass(
                    f1, length, dim, closed
                )
                trData[
                    i * reparamn + k, 0:length, dim : 2 * dim
                ] = randomCurveFromShapeClass(f2, length, dim, closed)
                trLabels[i * reparamn + k] = otrLabels[i]
            if verbose:
                print(
                    "Epoc: {}    Percent: {}".format(j, (i + 1) * 100 / trainsize),
                    end="\r",
                )
...
def trainAndValidateModelwSPDA(
    model,
    length,
    dim,
    closed,
    ...
    for j in range(0, e):
        ...
        for i in range(0, trainsize):
            ...
            for k in range(0, reparamn):
                ...
                trLabels[i * reparamn + k] = otrLabels[i]
            print(
                "Epoc: {}    Percent: {}".format(j, (i + 1) * 100 / trainsize), end="\r"
            )
        ...
        train = model.predict(
            x=[otrData[:, :, 0:dim], otrData[:, :, dim : 2 * dim]], verbose=0
        )
        ...
        # Save the model state to disk if test_MSE is better
        if j == 0 or testMSE[j] < min(testMSE[:j]):
...
class CyclicPad(Layer):
    ...
    def compute_output_shape(self, input_shape):
        return (
            input_shape[0],
            input_shape[1] + 2 * math.floor(self.kernalsize / 2),
            input_shape[2],
        )
